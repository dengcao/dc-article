...
//        }
    ...
    public function cacheClear()//清空缓存
    {
        Log::clear();//清空日志
        Cache::clear();//清空缓存
        $runtime_path=public_path()."runtime\\";
        array_map( 'unlink', glob( $runtime_path."admin\\".'temp\\'.'*.php' ) );//清除模版缓存 不删除 temp目录
        array_map( 'unlink', glob( $runtime_path."index\\".'temp\\'.'*.php' ) );//清除模版缓存 不删除 temp目录
