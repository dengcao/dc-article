...
class Article
{
    ...
    public function addSave()
    {
        ...
        $id = Db::name('article')->insertGetId($update_data);

        if($id>0){
            write_syslog(array("log_content"=>"新增文章，ID：".$id));//记录系统日志
    ...
    public function editSave()
    {
        ...
        $article=ArticleModel::where("id","=",$update_data["id"])->findOrEmpty();
        ...
        if($update_result){
            write_syslog(array("log_content"=>"修改文章，ID：".$update_data["id"]));//记录系统日志
    ...
    public function view()
    {
        ...
        $article=ArticleModel::where("id","=",$id)->with('category')->withAttr('status', function($value) {
    ...
    public function get()//获取文章数据
    {
        ...
        if(isset($action["keyword"])){
            if($action["keyword"]!=""){
                $list=$list->where("a.title|a.content|a.keywords|a.description|a.url|a.author|a.copyfrom","like","%".$action["keyword"]."%");
            }
        }
    ...
    public function delete()//删除数据
    {
        ...
        $del_num=0;
        if($id){
            $del_num=ArticleModel::where("id","in",$id)->delete();
        }
        if($del_num>0){
            write_syslog(array("log_content"=>"删除文章(ID)：".$id));//记录系统日志
